...
export default function AddPaymentPage() {
  ...
  const loadSale = async () => {
    try {
      ...
      console.error('Satış yüklenirken hata:', error);
      alert('Satış yüklenirken bir hata oluştu: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (amount > remainingAmount) {
      alert(`Ödeme tutarı kalan alacaktan (${formatCurrency(remainingAmount)}) fazla olamaz!`);
      return;
    }
    ...
    try {
      ...
      // Başarılı oldu, satış detay sayfasına yönlendir
      router.push(`/accounting/sales/${saleId}`);
    } catch (error: any) {
    ...
    <MainLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button
            onClick={() => router.push(`/accounting/sales/${saleId}`)}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      Kalan alacak: <span className="font-semibold text-red-600">{formatCurrency(remainingAmount)}</span>
                    </p>
                  </div>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{formatCurrency(sale.final_amount)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {formData.amount && parseFloat(formData.amount) > 0 && (
                    <>
                      ...
                      <div className="flex justify-between pt-2 border-t">
                        <span className="text-sm font-medium text-gray-700">Yeni Kalan Alacak:</span>
